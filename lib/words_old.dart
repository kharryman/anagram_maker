...
class WordsPageOldState extends State<WordsPageOld> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.isMultipleWords == true) {
      Map<int, List<List<dynamic>>> myMap =
          Map<int, List<List<dynamic>>>.from(widget.words[0]);
      for (int key in myMap.keys) {
        numberAnagrams += myMap[key]!.length;
      }
    } else {
      numberAnagrams = widget.words.length;
    }
    return Scaffold(
        appBar: AppBar(
          title: Text(
              "${FlutterI18n.translate(context, "APP_TITLE")}(${widget.countAnagrams})"),
          actions: <Widget>[
            ...
            children: <Widget>[
              Expanded(
                ...
                child: SizedBox(
                  ...
                  child: widget.isMultipleWords == true
                      ? ListMultiple(words: widget.words)
                      : ListSingle(words: widget.words),
                ),
              ))
            ]));
  }
